public final class Merge5Pipeline<Value, Source1, Source2, Source3, Source4, Source5>: Pipeline {
    ...
    private let merge: (Source1, Source2, Source3, Source4, Source5) async -> Value
    ...
    init(
        ...
        merge: @escaping (Source1, Source2, Source3, Source4, Source5) async -> Value
    ) async {
        ...
        await subscription1.addSubscription(listener: self) { [weak self] value in
            await self?.publish(value1: value, value2: source2.value(), value3: source3.value(), value4: source4.value(), value5: source5.value())
        }
        await subscription2.addSubscription(listener: self) { [weak self] value in
            await self?.publish(value1: source1.value(), value2: value, value3: source3.value(), value4: source4.value(), value5: source5.value())
        }
        await subscription3.addSubscription(listener: self) { [weak self] value in
            await self?.publish(value1: source1.value(), value2: source2.value(), value3: value, value4: source4.value(), value5: source5.value())
        }
        await subscription4.addSubscription(listener: self) { [weak self] value in
            await self?.publish(value1: source1.value(), value2: source2.value(), value3: source3.value(), value4: value, value5: source5.value())
        }
        await subscription5.addSubscription(listener: self) { [weak self] value in
            await self?.publish(value1: source1.value(), value2: source2.value(), value3: source3.value(), value4: source4.value(), value5: value)
        }
    }

    public func value() async -> Value {
        return await merge(source1.value(), source2.value(), source3.value(), source4.value(), source5.value())
    }

    private func publish(value1: Source1, value2: Source2, value3: Source3, value4: Source4, value5: Source5) async {
        let newValue = await merge(value1, value2, value3, value4, value5)
